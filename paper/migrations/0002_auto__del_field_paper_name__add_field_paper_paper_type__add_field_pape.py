# -*- coding: utf-8 -*-
from south.utils import datetime_utils as datetime
from south.db import db
from south.v2 import SchemaMigration
from django.db import models


class Migration(SchemaMigration):

    def forwards(self, orm):
        # Deleting field 'Paper.name'
        db.delete_column('paper', 'name')

        # Adding field 'Paper.paper_type'
        db.add_column('paper', 'paper_type',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['paper_type.PaperType']),
                      keep_default=False)

        # Adding field 'Paper.paper_weight'
        db.add_column('paper', 'paper_weight',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['paper_weight.PaperWeight']),
                      keep_default=False)

        # Adding field 'Paper.paper_finish'
        db.add_column('paper', 'paper_finish',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['paper_finish.PaperFinish'], null=True, blank=True),
                      keep_default=False)

        # Adding field 'Paper.price_per_kilogram'
        db.add_column('paper', 'price_per_kilogram',
                      self.gf('django.db.models.fields.DecimalField')(default=None, max_digits=11, decimal_places=2),
                      keep_default=False)


    def backwards(self, orm):
        # Adding field 'Paper.name'
        db.add_column('paper', 'name',
                      self.gf('django.db.models.fields.CharField')(default=None, max_length=1024),
                      keep_default=False)

        # Deleting field 'Paper.paper_type'
        db.delete_column('paper', 'paper_type_id')

        # Deleting field 'Paper.paper_weight'
        db.delete_column('paper', 'paper_weight_id')

        # Deleting field 'Paper.paper_finish'
        db.delete_column('paper', 'paper_finish_id')

        # Deleting field 'Paper.price_per_kilogram'
        db.delete_column('paper', 'price_per_kilogram')


    models = {
        u'paper.paper': {
            'Meta': {'ordering': "['-pk']", 'object_name': 'Paper', 'db_table': "'paper'"},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'paper_finish': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['paper_finish.PaperFinish']", 'null': 'True', 'blank': 'True'}),
            'paper_type': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['paper_type.PaperType']"}),
            'paper_weight': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['paper_weight.PaperWeight']"}),
            'price_per_kilogram': ('django.db.models.fields.DecimalField', [], {'max_digits': '11', 'decimal_places': '2'})
        },
        u'paper_finish.paperfinish': {
            'Meta': {'ordering': "['-pk']", 'object_name': 'PaperFinish', 'db_table': "'paper_finish'"},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '1024'})
        },
        u'paper_type.papertype': {
            'Meta': {'ordering': "['-pk']", 'object_name': 'PaperType', 'db_table': "'paper_type'"},
            'better_quality_paper': ('django.db.models.fields.BooleanField', [], {}),
            'has_finish': ('django.db.models.fields.BooleanField', [], {}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '1024'})
        },
        u'paper_weight.paperweight': {
            'Meta': {'ordering': "['-pk']", 'object_name': 'PaperWeight', 'db_table': "'paper_weight'"},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'weight': ('django.db.models.fields.IntegerField', [], {})
        }
    }

    complete_apps = ['paper']